...
public abstract class LazyAssessNN implements Comparable<LazyAssessNN> {
    ...
    protected SequenceStatsCache cache;               // Cache to store the information for the sequences
    protected Sequence query, reference;              // Query and reference sequences
    protected int indexStoppedLB, oldIndexStoppedLB;  // Index where we stop LB
    protected int indexStoppedED;

    protected double minDist;                         // distance
    protected double bestMinDist;                     // best so far distance
    protected double kimDist;
    public double euclideanDistance;
    public double upperBoundDistance;
    public double nnDistance;

    protected LBStatus status;                        // Status of Lower Bound
    ...
    public int getMinWindowValidityForFullDistance() {
        if (status == LBStatus.Full_DTW || status == LBStatus.Full_ERP || status == LBStatus.Full_LCSS) {
