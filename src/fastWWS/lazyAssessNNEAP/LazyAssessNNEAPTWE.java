 ...
 */
public class LazyAssessNNEAPTWE extends LazyAssessNN {
    private final EAPTWE distComputer = new EAPTWE();
    protected double currentNu, currentLambda;

    public LazyAssessNNEAPTWE(final SequenceStatsCache cache) {
        super(cache);
    }

    public LazyAssessNNEAPTWE(final Sequence query, final int index,
    ...
    public RefineReturnType tryToBeat(final double scoreToBeat, final double nu, final double lambda) {
        ...
        switch (status) {
            ...
            case Full_LB_TWE:
                if (bestMinDist >= scoreToBeat) return RefineReturnType.Pruned_with_LB;
                minDist = distComputer.distance(query.data[0], reference.data[0], nu, lambda, Double.POSITIVE_INFINITY);
