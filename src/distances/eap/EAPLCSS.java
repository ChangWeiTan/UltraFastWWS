...
public class EAPLCSS extends ElasticDistances {
        ...
        for (; i < nblines; ++i) {
            ...
            for (j = jStart; j < jStop; ++j) {
                if (sim(li, cols[j], epsilon)) {
                    ...
                    buffers[c + j] = GenericTools.max3(buffers[c + j - 1], buffers[p + j - 1], buffers[p + j]);
                }
            }
        }
    ...
    public WarpingPathResults distanceExt(double[] lines, double[] cols, double epsilon, int w, double cutoff) {
        // Ensure that lines are longer than columns
        if (lines.length < cols.length) {
            double[] swap = lines;
            lines = cols;
        ...
        for (; i < nblines; ++i) {
            ...
            for (j = jStart; j < jStop; ++j) {
                ...
                if (sim(li, cols[j], epsilon)) {
                    ...
                    current_max = max(current_max, cost);
                    buffers[c + j] = cost;
                    buffers_w[c + j] = max(dw, buffers_w[p + j - 1]);
                } else { // Note: Diagonal lookup required, e.g. when w=0
                    if (w == 0) {
                        buffers[c + j] = GenericTools.max3(buffers[c + j - 1], buffers[p + j - 1], buffers[p + j]);
                        buffers_w[c + j] = 0;
                    } else if (buffers[p + j] > buffers[c + j - 1]) {
                        buffers[c + j] = buffers[p + j];
                        buffers_w[c + j] = buffers_w[p + j];
                    } else {
                        buffers[c + j] = buffers[c + j - 1];
                        buffers_w[c + j] = buffers_w[c + j - 1];
                    }
                }
            }
        }
        ...
        // --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- --- ---
        // Finalisation: put the result on a [0 - 1] range
        return new WarpingPathResults(1.0 - (double) (buffers[c + nbcols - 1]) / (double) nbcols, buffers_w[c + nbcols - 1]);
    }
}
